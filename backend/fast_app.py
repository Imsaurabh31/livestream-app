...
def video_feed():
    def generate():
        import random
        colors = [b'\xff\x00\x00', b'\x00\xff\x00', b'\x00\x00\xff']  # RGB colors
        
        while streaming:
            # Create a simple colored square that changes
            color = random.choice(colors)
            
            # Minimal valid JPEG header + data + footer
            jpeg_data = (
...
def create_overlay():
    ...
    return jsonify({'id': data['_id']}), 201
